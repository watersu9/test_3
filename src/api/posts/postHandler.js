...
const createPost = async (req, res) => {
    ...
    try {
        ...
        const results = await db.executeQuery(query, [title, content, user_id, photo_url]);
...
const updatePost = async (req, res) => {
    ...
    try {
        ...
        const result = await db.executeQuery(query, [title, content, post_id]);
...
const likePost = async (req, res) => {
    ...
    const {user_id} = req.decoded; // user_id 받기 

    try {
        // 게시물의 좋아요 정보를 조회합니다.
        const query_1 = 'SELECT likes FROM posts WHERE post_id = ?';
        const post = await db.executeQuery(query_1, [post_id]);
...
const listPosts = async (req, res, next) => {
    ...
    try {
        ...
        const postQuery = 'SELECT * FROM posts ORDER BY post_id DESC LIMIT ? OFFSET ?';
        const countQuery = 'SELECT COUNT(*) as count FROM posts';

        // 데이터베이스 쿼리 실행
        const posts = await db.executeQuery(postQuery, [limit, offset]);
