...
const register = async (req, res, next) => {
  try {
    ...
    const query = `INSERT INTO users (user_id, username, password,email) VALUES (?, ?, ?, ?)`;

    
    // executeQuery 함수를 사용하여 쿼리를 실행합니다.
    await db.executeQuery(query, [user_id, username, password, email]);
...
const login = async (req, res, next) => {
  ...
  const data = await db.runQuery(`SELECT * FROM users WHERE user_id = '${user_id}' and password = '${password}';`)
  console.log(data)
  if (data && data.length) {
    // 성공
    const user = data[0]
...
const requestCoaching = async (req, res, next) => {
  try {
    ...
    await db.executeQuery(query, [user_id, trainer_id, today, interaction_type]);
    return res.status(200).json({
      code: 200,
